...
const TempUser = require('../db/model');
...
passport.use(new GoogleStrategy({
    ...
    async (accessToken,refreshToken,profile,done) => {
        ...
        const user = await TempUser.findOne({googleID:googleID});
        if(!user){
            console.log('Creating new user');
            const newUser = await TempUser.create({username:username,googleID:googleID,isAdmin:isAdmin});
            done(null,newUser);
        }
        else{
